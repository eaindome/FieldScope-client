<script lang="ts">
	...
	async function loadData() {
		...
		// Load forms
		const formsResult = await api.getProjectForms(projectId);
		if ('error' in formsResult && formsResult.error) {
			error = formsResult.error;
			loading = false;
			return;
		}

		form = (formsResult.data || []).find((f: any) => f.id === formId);
		if (!form) {
	...
	const isProjectEnded = $derived.by(() => {
	...
	async function handleSubmit() {
		...
		try {
			...
			if (isEditMode && existingSubmission) {
				...
				const { data, error: submitError } = await api.submitData({
					projectId,
					formId,
					answers: plainAnswers,
					localSyncId: crypto.randomUUID()
				});
	...
	const completedFields = $derived(
		allAnswerableFields.filter((f: any) => {
	...
	<div class="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-slate-200 shadow-lg z-50 lg:relative lg:z-0 lg:border-t-0 lg:shadow-none lg:mt-6">
		...
		{:else}
			<Button
				onclick={handleSubmit}
				loading={submitting}
				disabled={submitting || isProjectEnded}
			...
			{#if isProjectEnded}
				<p class="text-xs text-center text-amber-600 mt-2">This project has ended. Submissions are no longer accepted.</p>
			{/if}
		{/if}

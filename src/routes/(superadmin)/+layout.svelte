<script lang="ts">
	...
	let systemExpanded = $state($page.url.pathname.startsWith('/system'));
	...
	const navItems = [
		...
		{ href: '/invitations', label: 'Invitations', icon: 'Mail' },
		{
			...
			children: [
				...
				{ href: '/system/health', label: 'Health', icon: 'Heart' }
			]
		}
	];
	...
	$effect(() => {
		// Auto-expand System if on a system page
			...
			<div class="absolute top-full left-0 right-0 bg-white border-b border-slate-200 shadow-lg">
				<nav class="px-4 py-2">
					...
					<div class="px-3 py-4 text-center text-slate-600 text-sm">
						...
						<p class="text-xs">Use desktop to access admin features</p>
					</div>
					<button
			...
			<div class="flex items-center {sidebarCollapsed ? 'justify-center' : 'gap-3'}">
				...
				{#if !sidebarCollapsed}
					<div>
						...
						<p class="text-xs text-slate-400 mb-1">SuperAdmin Panel</p>
					</div>
				{/if}
			</div>
		</div>
					...
					<li>
						{#if item.isDropdown}
							...
							<button
								...
								class="w-full flex items-center {sidebarCollapsed ? 'justify-center' : 'justify-between'} px-4 py-3 rounded-lg transition-all text-slate-300 hover:bg-slate-800 hover:text-white"
							...
							>
								...
								{#if !sidebarCollapsed}
									<div class={`transition-transform duration-200 ${systemExpanded ? 'rotate-180' : ''}`}>
										{@html icons.ChevronDown(16)}
									</div>
								{/if}
							</button>

							{#if systemExpanded && item.children && !sidebarCollapsed}
								<ul class="mt-1 ml-8 space-y-1 border-l-2 border-slate-800 pl-4">
										...
										<li>
											...
											>
												<div class="w-4 h-4 flex items-center justify-center">
													{@html icons[child.icon as keyof typeof icons](16)}
												</div>
												{child.label}
											</a>
										</li>
						...
						{:else}
							...
							<a
								href={item.href}
								class={`flex items-center ${sidebarCollapsed ? 'justify-center' : 'gap-3'} px-4 py-3 rounded-lg transition-all ${
									isActive(item.href)
...
<Dialog bind:open={showLogoutDialog} onClose={() => showLogoutDialog = false} title="Confirm Logout" description="Are you sure you want to logout?">
	<div class="space-y-4">
		<p class="text-sm text-slate-600">You will need to sign in again to access the dashboard.</p>

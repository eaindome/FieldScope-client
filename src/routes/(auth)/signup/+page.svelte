<script lang="ts">
	...
	async function handleSignup(e: Event) {
		...
		const { data, error: err } = await api.register(email, password, name);
...
<div>
	<div class="mb-8">
		<h1 class="text-3xl font-bold text-slate-900 tracking-tight bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text">Create your account</h1>
		<p class="text-slate-600 text-sm mt-2">Sign up to get started with FieldScope</p>
	</div>
	...
	{#if error}
		<div class="mb-5 flex items-start gap-3 bg-red-50 border border-red-200 rounded-xl p-3.5 animate-shake">
			...
			<div>
				<p class="text-sm font-medium text-red-800">Sign up failed</p>
	...
	<form onsubmit={handleSignup} class="space-y-5">
		<div class="space-y-2 group">
			<Label for="name" class="text-sm font-medium text-slate-700 transition-colors group-focus-within:text-blue-600">Full name</Label>
			<Input
				id="name"
				type="text"
				bind:value={name}
			...
			{#if password.length > 0}
				<div class="space-y-1.5">
					<div class="flex items-center justify-between">
						...
						<span class="text-xs font-medium {passwordStrength().label === 'Strong' ? 'text-green-600' : passwordStrength().label === 'Good' ? 'text-yellow-600' : passwordStrength().label === 'Fair' ? 'text-orange-600' : 'text-red-600'}">{passwordStrength().label}</span>
					</div>
					<div class="h-1.5 bg-slate-200 rounded-full overflow-hidden">
						<div
							class="h-full {passwordStrength().color} transition-all duration-300 rounded-full"
		...
		<div class="space-y-2 group">
			...
			<div class="relative">
				<Input
					...
					required
					disabled={loading}
					class="pr-10 transition-all duration-200 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 focus:shadow-lg focus:shadow-blue-500/10 {confirmPassword && password !== confirmPassword ? 'border-red-300' : ''}"
					autocomplete="new-password"
				/>
				<button
					type="button"
		...
		<div class="flex items-start gap-2 pt-1">
			<input
				...
				id="terms"
				required
				class="mt-1 w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500 focus:ring-2 cursor-pointer transition-all duration-200"
			/>
			<label for="terms" class="text-sm text-slate-600 cursor-pointer select-none">
				I agree to the <a href="/terms" class="text-blue-600 hover:text-blue-700 font-medium transition-all duration-200 hover:underline hover:underline-offset-2">Terms of Service</a> and <a href="/privacy" class="text-blue-600 hover:text-blue-700 font-medium transition-all duration-200 hover:underline hover:underline-offset-2">Privacy Policy</a>
			</label>
		</div>
	...
	<div class="mt-6 text-center">
		<p class="text-sm text-slate-600">
			...
			<a href="/login" class="text-blue-600 hover:text-blue-700 font-semibold ml-1 transition-all duration-200 hover:underline hover:underline-offset-2">
				Sign in
			</a>
		</p>
	</div>
